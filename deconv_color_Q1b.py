...
if __name__ == '__main__':
    ...
    img_rgb = img_rgb[int(roi_rgb[1]):int(roi_rgb[1]+roi_rgb[3]), int(roi_rgb[0]):int(roi_rgb[0]+roi_rgb[2])]
    ...
    cv2.createTrackbar('d', win, int(opts.get('--d', 22)), 70, update)
    cv2.createTrackbar('SNR (db)', win, int(opts.get('--snr', 25)), 100, update)
    update(None)
